...
import os, json, logging, cv2, argparse, pickle, re
...
QUERY_DIR = os.environ.get("DINO_QUERY_DIR", os.path.join(PROJECT_ROOT, "data", "query_images"))
GT_JSON = os.environ.get("DINO_GT_JSON", os.path.join(PROJECT_ROOT, "data", "ground_truth.json"))
FEATURES_BASE = os.environ.get("DINO_FEATURES_BASE", os.path.join(PROJECT_ROOT, "data", "features_yolo_v2"))
RESULTS_DIR = os.environ.get("DINO_RESULTS_DIR", os.path.join(PROJECT_ROOT, "evaluation_results"))
...
def _canonical_stem(x):
    stem = _stem(x)
    match = re.match(r"(.+)_page_?(\d+)(?:_(\d+))?$", stem)
    if not match:
        return stem
    catalogue, page, crop = match.groups()
    return f"{catalogue}__pdf-{int(page):04d}__crop-{int(crop or 0):02d}"
...
def load_gt_dual_keys(path):
    ...
    def add(q, v):
        ...
        vset |= {_canonical_stem(x) for x in v if x}
...
# ============================================================================
def main():
    global QUERY_DIR, GT_JSON, FEATURES_BASE, RESULTS_DIR
    ...
    parser.add_argument('--gt-json', default=GT_JSON, help='Ground-truth JSON used for retrieval metrics')
