# Enhanced app.py with improved interface and features
import gradio as gr
import os
import json
import pickle
...
# ---------------- ENHANCED CONFIG ----------------
FEATURES_BASE = "<HPC_DATA_ROOT>/<HPC_ACCOUNT>/restored_data/Experiment/dino/features_methods/multilyer_fusion_pca/claude/finalizing_20k_to_total/features_c4_c5_average_new"
METHOD_NAME = "C4_C5_average_gem_c1_pca1024_pytorch"
IMAGE_DIRS = {
    "20K": "<HPC_DATA_ROOT>/<HPC_ACCOUNT>/preparation/crops_20k",
    "413K": "<HPC_DATA_ROOT>/<HPC_ACCOUNT>/preparation/crops_new"
}
QUERY_DIR = "<HPC_DATA_ROOT>/<HPC_ACCOUNT>/restored_data/auction-retrieval/quer_2/query_images"
...
class MemoryEfficientCache:
    """Memory-efficient cache for database features"""
...
# ---------------- Average Fusion Function ----------------
def apply_consistent_average_fusion_for_queries(c4_feat, c5_feat, device='cpu'):
    """Apply CONSISTENT average fusion for query features - MUST match training"""
    
    # L2 normalize each layer
    c4_norm = F.normalize(c4_feat, p=2, dim=1, eps=1e-8)
    c5_norm = F.normalize(c5_feat, p=2, dim=1, eps=1e-8)
    
    # FIXED: Use SAME deterministic expansion as training
    c4_dim = c4_norm.shape[1]  # 1024
    c5_dim = c5_norm.shape[1]  # 2048
    
    if c4_dim != c5_dim:
        if c5_dim == 2 * c4_dim:
            # Simply repeat C4 features to match C5 dimensions (same as training)
            c4_expanded = torch.cat([c4_norm, c4_norm], dim=1)
        else:
            # Use zero-padding to match dimensions (same as training)
...
def apply_pca_from_dict(pca_data, X):
    """Apply PCA transform using loaded dict data with enhanced error handling"""
    if pca_data is None:
    ...
    try:
        ...
        # Handle both dictionary and sklearn-like formats
...
def preprocess_c1_with_steps(img_input):
    """C1 preprocessing with step-by-step visualization - Fixed version"""
    try:
        # Step 0: Input validation and conversion
        ...
        # Step 2: RGB to BGR conversion (for OpenCV)
        img_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        img_bgr_display = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)  # Convert back for display
        steps.append({
        ...
        # Step 4: Grayscale to RGB (normalization)
        ...
        # Return original and error step
        try:
...
def preprocess_c1_pil(img_input):
    """Original C1 preprocessing function - now uses step-by-step version internally"""
    try:
...
def get_query_images():
    """Get list of preloaded query images"""
...
def get_available_datasets():
    """Get list of available datasets with validation"""
...
def load_consolidated_db_features(dataset_name):
    """Load consolidated database features and metadata with caching"""
...
def search_topk_cosine_only(q: np.ndarray, db: np.ndarray, k: int):
    """Search top-k similar images using COSINE similarity only"""
    if db.size == 0:
        return None, None
    ...
    if FAISS_OK:
        # Use FAISS for cosine similarity (Inner Product on normalized vectors)
...
def encode_query(image_input, model: nn.Module, pca_model=None) -> np.ndarray:
    """Encode query image using the same pipeline - handles PIL Image or numpy array"""
    try:
...
def search_with_scores(image_input, dataset_name: str, topk: int):
    """Main search function with enhanced score display"""
    ...
    try:
        ...
        # Get image directory for dataset
        ...
        # Search using COSINE similarity only
        sims, idxs = search_topk_cosine_only(q, db_feats, topk)
        if sims is None:
            return [], "❌ Search failed"
        
        # Prepare results with enhanced captions
        results = []
        for rank, (i, s) in enumerate(zip(idxs, sims), 1):
            ...
            # Enhanced caption with score and ranking
...
def process_and_show_query_detailed(image_input):
    """Process query image with detailed step-by-step visualization"""
    if image_input is None:
        return None, None, "", [], ""
...
def process_and_show_query(image_input):
    """Simplified processing preview (for compatibility)"""
    if image_input is None:
...
def get_random_query():
    """Get a random query image for testing"""
...
def create_enhanced_interface():
    """Create enhanced Gradio interface with modern design"""
    ...
    # Create main interface with modern theme
    with gr.Blocks(
        ...
        with gr.Row():
            # Left Column - Enhanced Query Management
            with gr.Column(scale=2):
                ...
                with gr.Tabs():
                    ...
                    with gr.Tab("🔬 Step-by-Step Analysis"):
                        ...
                        # Technical details with better styling
                        technical_details = gr.Markdown("", elem_classes=["technical-details"])
                
                # Main query image display
            ...
            # Right Column - Results and Tools
        ...
        def update_current_query_from_upload(image):
            """Update current query from uploaded image"""
...
"""
        ...
        # Update analysis when search completes
        ...
        # Auto-load a random query on startup for demo
        if query_images:
            demo.load(
                fn=load_random_query,
                outputs=[current_query, original_preview, processed_preview, process_status]
